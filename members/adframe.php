...
if ($numrows == 1) {
    ...
    if (!$numrows1) {
    		$top = "adtop.php?message=".urlencode("Invalid click link");
    }  //end if ($numrows1 ==1)
    else {
	
		$paid = mysql_result($result1, 0, "paid");
		
		if($paid) $top = "adtop.php?message=".urlencode("You have already received credit for this link")."&url=".urlencode($url);
		else {
			...
			$referid = mysql_result($sql, 0,'referid');
				...
				$top = "adtop.php?id=".$id."&type=pro&url=".urlencode($url);
